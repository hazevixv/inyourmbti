    ...
    <main className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-teal-50 md:pb-8 pt-20">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <header className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 md:p-10 shadow-xl mb-8 text-center">
          <div className="w-20 h-20 md:w-24 md:h-24 rounded-3xl bg-gradient-to-br from-pink-400 to-purple-500 flex items-center justify-center shadow-2xl mx-auto mb-6">
            <Heart className="w-10 h-10 md:w-12 md:h-12 text-white" />
          </div>
          <h1 className="text-3xl md:text-5xl font-black mb-4 text-navy-900">
            Compatibility Checker
          </h1>
          <p className="text-lg md:text-xl text-navy-600 max-w-2xl mx-auto">
        ...
        <section className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-lg mb-8">
          <h2 className="text-xl font-bold mb-6 text-navy-800 text-center">Select Two Types to Compare</h2>
          
          <div className="grid md:grid-cols-3 gap-4 items-center">
            {/* Type 1 */}
            <div>
              <label className="block text-sm font-semibold text-navy-700 mb-2">First Type</label>
              <select 
                value={type1} 
                onChange={(e) => setType1(e.target.value as PersonalityCode)}
                className="w-full px-4 py-3 border-2 border-sky-200 rounded-xl bg-white text-navy-800 font-bold text-lg focus:border-sky-400 focus:outline-none transition"
              >
                {types.map(code => (
            ...
            {/* Type 2 */}
            <div>
              <label className="block text-sm font-semibold text-navy-700 mb-2">Second Type</label>
              <select 
                value={type2} 
                onChange={(e) => setType2(e.target.value as PersonalityCode)}
                className="w-full px-4 py-3 border-2 border-teal-200 rounded-xl bg-white text-navy-800 font-bold text-lg focus:border-teal-400 focus:outline-none transition"
              >
                {types.map(code => (
                  <option key={code} value={code}>
                    {code} - {getPersonalityType(code)?.nickname}
        ...
        {compatibility && (
          <>
            {/* Score Card */}
            <section className={`bg-gradient-to-br ${getLevelColor(compatibility.level)} rounded-3xl p-8 md:p-10 shadow-2xl mb-8 text-white text-center`}>
              <div className="text-6xl mb-4">{getLevelEmoji(compatibility.level)}</div>
              <div className="text-7xl md:text-8xl font-black mb-2">{compatibility.score}</div>
              <div className="text-2xl md:text-3xl font-bold mb-4 uppercase tracking-wide">
                {compatibility.level}
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-8">
              <Link
                ...
                className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 shadow-lg hover:shadow-xl hover:scale-105 transition"
              >
                <div className="flex items-center gap-4 mb-3">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-sky-400 to-teal-400 flex items-center justify-center text-white font-bold">
                    {type1}
                  </div>
                  <div>
                    <div className="font-bold text-navy-800">{type1Data?.nickname}</div>
                    <div className="text-sm text-navy-600">{type1Data?.category}</div>
                  </div>
                </div>
                <p className="text-sm text-navy-600 line-clamp-2">{type1Data?.tagline}</p>
              </Link>
              ...
              >
                <div className="flex items-center gap-4 mb-3">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-teal-400 to-navy-500 flex items-center justify-center text-white font-bold">
                    {type2}
                  </div>
                  <div>
                    <div className="font-bold text-navy-800">{type2Data?.nickname}</div>
                    <div className="text-sm text-navy-600">{type2Data?.category}</div>
                  </div>
                </div>
                <p className="text-sm text-navy-600 line-clamp-2">{type2Data?.tagline}</p>
              </Link>
            </div>
            ...
            <section className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-lg mb-6">
              ...
              <div className="space-y-3">
                {compatibility.challenges.map((challenge, i) => (
                  <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-orange-50 border border-orange-200">
            ...
            <section className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-lg mb-8">
              ...
              <div className="p-6 rounded-xl bg-gradient-to-r from-yellow-50 to-orange-50 border-l-4 border-yellow-500">
                <p className="text-lg text-navy-700 leading-relaxed">{compatibility.advice}</p>
              </div>
            </section>
          </>
        )}
        ...
        <section className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-lg mb-8">
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="font-bold text-navy-800">Excellent (85-100)</div>
                <div className="text-sm text-navy-600">Natural harmony, complementary strengths, minimal friction</div>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="font-bold text-navy-800">Good (70-84)</div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="font-bold text-navy-800">Moderate (55-69)</div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="font-bold text-navy-800">Challenging (&lt;55)</div>
